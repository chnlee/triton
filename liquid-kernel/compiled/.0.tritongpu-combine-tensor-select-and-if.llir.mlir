...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:75", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @matmul_kernel(%arg0: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %arg3: i32 {tt.divisibility = 16 : i32}, %arg4: i32 {tt.divisibility = 16 : i32}, %arg5: i32 {tt.divisibility = 16 : i32}, %arg6: i32 {tt.divisibility = 16 : i32}, %arg7: i32 {tt.divisibility = 16 : i32}, %arg8: i32 {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    ...
    %53:3 = scf.for %arg9 = %c0_i32 to %50 step %c1_i32 iter_args(%arg10 = %cst, %arg11 = %38, %arg12 = %48) -> (tensor<128x64xf32, #blocked>, tensor<128x32x!tt.ptr<f32>, #blocked2>, tensor<32x64x!tt.ptr<f32>, #blocked1>)  : i32 {
      ...
      %73 = tt.splat %72 : i32 -> tensor<1x32xi32, #blocked2>
      %74 = arith.cmpi slt, %33, %73 : tensor<1x32xi32, #blocked2>
      %75 = tt.broadcast %74 : tensor<1x32xi1, #blocked2> -> tensor<128x32xi1, #blocked2>
      %76 = tt.load %arg11, %75, %cst_1 : tensor<128x32x!tt.ptr<f32>, #blocked2>
      %77 = ttg.local_alloc %76 : (tensor<128x32xf32, #blocked2>) -> !ttg.memdesc<128x32xf32, #shared, #smem>
      %78 = tt.splat %72 : i32 -> tensor<32x1xi32, #blocked1>
      %79 = arith.cmpi slt, %40, %78 : tensor<32x1xi32, #blocked1>
      %80 = tt.broadcast %79 : tensor<32x1xi1, #blocked1> -> tensor<32x64xi1, #blocked1>
      %81 = tt.load %arg12, %80, %cst_0 : tensor<32x64x!tt.ptr<f32>, #blocked1>
      %82 = ttg.local_alloc %81 : (tensor<32x64xf32, #blocked1>) -> !ttg.memdesc<32x64xf32, #shared, #smem>
      %83 = ttg.local_load %77 : !ttg.memdesc<128x32xf32, #shared, #smem> -> tensor<128x32xf32, #ttg.dot_op<{opIdx = 0, parent = #blocked}>>
      %84 = ttg.local_load %82 : !ttg.memdesc<32x64xf32, #shared, #smem> -> tensor<32x64xf32, #ttg.dot_op<{opIdx = 1, parent = #blocked}>>
      %85 = tt.dot %83, %84, %arg10, inputPrecision = tf32 : tensor<128x32xf32, #ttg.dot_op<{opIdx = 0, parent = #blocked}>> * tensor<32x64xf32, #ttg.dot_op<{opIdx = 1, parent = #blocked}>> -> tensor<128x64xf32, #blocked>
      %86 = tt.addptr %arg11, %cst_2 : tensor<128x32x!tt.ptr<f32>, #blocked2>, tensor<128x32xi32, #blocked2>
      %87 = tt.addptr %arg12, %52 : tensor<32x64x!tt.ptr<f32>, #blocked1>, tensor<32x64xi32, #blocked1>
      scf.yield %85, %86, %87 : tensor<128x64xf32, #blocked>, tensor<128x32x!tt.ptr<f32>, #blocked2>, tensor<32x64x!tt.ptr<f32>, #blocked1>
    }
    ...
    %70 = ttg.convert_layout %53#0 : tensor<128x64xf32, #blocked> -> tensor<128x64xf32, #blocked1>
