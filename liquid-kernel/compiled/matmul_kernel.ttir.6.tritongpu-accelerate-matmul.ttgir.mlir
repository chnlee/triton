...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:86", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @matmul_kernel(%arg0: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %arg3: i32 {tt.divisibility = 16 : i32}, %arg4: i32 {tt.divisibility = 16 : i32}, %arg5: i32 {tt.divisibility = 16 : i32}, %arg6: i32 {tt.divisibility = 16 : i32}, %arg7: i32 {tt.divisibility = 16 : i32}, %arg8: i32 {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    ...
    %55:3 = scf.for %arg9 = %c0_i32 to %52 step %c1_i32 iter_args(%arg10 = %cst, %arg11 = %38, %arg12 = %50) -> (tensor<128x64xf32, #blocked>, tensor<128x32x!tt.ptr<f32>, #blocked2>, tensor<32x64x!tt.ptr<f32>, #blocked1>)  : i32 {
      %73 = arith.muli %arg9, %c32_i32 : i32
      %74 = arith.subi %arg5, %73 : i32
      %75 = tt.splat %74 : i32 -> tensor<1x32xi32, #blocked2>
      %76 = arith.cmpi slt, %33, %75 : tensor<1x32xi32, #blocked2>
      %77 = tt.broadcast %76 : tensor<1x32xi1, #blocked2> -> tensor<128x32xi1, #blocked2>
      %78 = tt.load %arg11, %77, %cst_1 : tensor<128x32x!tt.ptr<f32>, #blocked2>
      ...
      %82 = tt.load %arg12, %81, %cst_0 : tensor<32x64x!tt.ptr<f32>, #blocked1>
      %83 = ttg.convert_layout %78 : tensor<128x32xf32, #blocked2> -> tensor<128x32xf32, #ttg.dot_op<{opIdx = 0, parent = #blocked}>>
      %84 = ttg.convert_layout %82 : tensor<32x64xf32, #blocked1> -> tensor<32x64xf32, #ttg.dot_op<{opIdx = 1, parent = #blocked}>>
      %85 = ttg.convert_layout %arg10 : tensor<128x64xf32, #blocked> -> tensor<128x64xf32, #mma>
      %86 = ttg.convert_layout %83 : tensor<128x32xf32, #ttg.dot_op<{opIdx = 0, parent = #blocked}>> -> tensor<128x32xf32, #ttg.dot_op<{opIdx = 0, parent = #mma, kWidth = 1}>>
      %87 = ttg.convert_layout %84 : tensor<32x64xf32, #ttg.dot_op<{opIdx = 1, parent = #blocked}>> -> tensor<32x64xf32, #ttg.dot_op<{opIdx = 1, parent = #mma, kWidth = 1}>>
      %88 = tt.dot %86, %87, %85, inputPrecision = tf32 : tensor<128x32xf32, #ttg.dot_op<{opIdx = 0, parent = #mma, kWidth = 1}>> * tensor<32x64xf32, #ttg.dot_op<{opIdx = 1, parent = #mma, kWidth = 1}>> -> tensor<128x64xf32, #mma>
      %89 = ttg.convert_layout %88 : tensor<128x64xf32, #mma> -> tensor<128x64xf32, #blocked>
      %90 = tt.addptr %arg11, %cst_2 : tensor<128x32x!tt.ptr<f32>, #blocked2>, tensor<128x32xi32, #blocked2>
      %91 = tt.addptr %arg12, %54 : tensor<32x64x!tt.ptr<f32>, #blocked1>, tensor<32x64xi32, #blocked1>
      scf.yield %89, %90, %91 : tensor<128x64xf32, #blocked>, tensor<128x32x!tt.ptr<f32>, #blocked2>, tensor<32x64x!tt.ptr<f32>, #blocked1>
    }
    ...
    %72 = ttg.convert_layout %55#0 : tensor<128x64xf32, #blocked> -> tensor<128x64xf32, #blocked1>
