...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:86", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @matmul_kernel(%arg0: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %arg3: i32 {tt.divisibility = 16 : i32}, %arg4: i32 {tt.divisibility = 16 : i32}, %arg5: i32 {tt.divisibility = 16 : i32}, %arg6: i32 {tt.divisibility = 16 : i32}, %arg7: i32 {tt.divisibility = 16 : i32}, %arg8: i32 {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    ...
    %59:3 = scf.for %arg9 = %c0_i32 to %56 step %c1_i32 iter_args(%arg10 = %cst, %arg11 = %40, %arg12 = %54) -> (tensor<128x64xf32, #blocked>, tensor<128x32x!tt.ptr<f32>, #blocked1>, tensor<32x64x!tt.ptr<f32>, #blocked>)  : i32 {
      ...
      %85 = arith.cmpi slt, %34, %84 : tensor<1x32xi32, #blocked1>
      %86 = tt.broadcast %85 : tensor<1x32xi1, #blocked1> -> tensor<128x32xi1, #blocked1>
      %87 = tt.load %arg11, %86, %cst_2 : tensor<128x32x!tt.ptr<f32>, #blocked1>
      %88 = tt.splat %83 : i32 -> tensor<32x1xi32, #blocked4>
      %89 = arith.cmpi slt, %43, %88 : tensor<32x1xi32, #blocked4>
      %90 = tt.broadcast %89 : tensor<32x1xi1, #blocked4> -> tensor<32x64xi1, #blocked4>
      %91 = ttg.convert_layout %90 : tensor<32x64xi1, #blocked4> -> tensor<32x64xi1, #blocked>
      %92 = tt.load %arg12, %91, %cst_1 : tensor<32x64x!tt.ptr<f32>, #blocked>
      %93 = ttg.convert_layout %87 : tensor<128x32xf32, #blocked1> -> tensor<128x32xf32, #ttg.dot_op<{opIdx = 0, parent = #blocked6}>>
      %94 = ttg.convert_layout %92 : tensor<32x64xf32, #blocked> -> tensor<32x64xf32, #ttg.dot_op<{opIdx = 1, parent = #blocked6}>>
      %95 = ttg.convert_layout %arg10 : tensor<128x64xf32, #blocked> -> tensor<128x64xf32, #blocked6>
      %96 = tt.dot %93, %94, %95, inputPrecision = tf32 : tensor<128x32xf32, #ttg.dot_op<{opIdx = 0, parent = #blocked6}>> * tensor<32x64xf32, #ttg.dot_op<{opIdx = 1, parent = #blocked6}>> -> tensor<128x64xf32, #blocked6>
      %97 = ttg.convert_layout %96 : tensor<128x64xf32, #blocked6> -> tensor<128x64xf32, #blocked>
      %98 = tt.addptr %arg11, %cst_0 : tensor<128x32x!tt.ptr<f32>, #blocked1>, tensor<128x32xi32, #blocked1>
      %99 = tt.addptr %arg12, %58 : tensor<32x64x!tt.ptr<f32>, #blocked>, tensor<32x64xi32, #blocked>
      scf.yield %97, %98, %99 : tensor<128x64xf32, #blocked>, tensor<128x32x!tt.ptr<f32>, #blocked1>, tensor<32x64x!tt.ptr<f32>, #blocked>
    }
    ...
    tt.store %73, %59#0, %81 : tensor<128x64x!tt.ptr<f32>, #blocked>
    tt.return
  }
}
